...
# 情况 B：详情页（?page=detail&rid=xxx）
    ...
    if rid:
        ...
        st.stop()
📌 修改说明
删除了第二段冗余循环，避免页面重复展示导致错位。

在第一段循环内直接处理分数映射（score_map），确保每题显示格式统一。

增加了 raw_str = None 初始化，避免变量未定义错误。

保留了长度警告，但不会阻断展示。

现在详情页将只显示一次完整的85道题，多选答案也会正确出现在对应题号下（前提是新测评数据已用修复后的 fmt_internal 函数生成）。对于旧数据，警告会提醒用户可能错位，但至少不会因为重复循环而进一步混乱。

还是没有处理掉
我们得仔细分析为什么多选答案仍然没有正确对应。用户提供的错误截图显示第80题显示“上父母课”，但第80题的题干是“为了解决问题，您之前尝试过哪些方式？（多选）”，这实际上是第80题（索引79）的内容，但答案却是“上父母课”，而第81题的题干是“您之前尝试的方法没有彻底生效的原因是？（多选）”，答案却显示“心理咨询”。这说明 答案整体向后偏移了一位，即第79题（索引78）的答案跑到了第80题的位置。

根本原因依然是在 ans_list 构建时，由于 fmt_internal 未正确处理逗号导致多选答案被拆分，但现在用户已经按照之前的建议修改了 fmt_internal 并重新测评了吗？用户说“还是没有处理掉”，可能有两种情况：

用户修改代码后，旧数据依然错位（因为旧数据本身已被错误拆分存储）。

用户测试时使用的是旧链接，没有重新测评生成新数据。

如果是新数据仍然错位，则需进一步排查 fmt_internal 是否真的生效，或者存储时是否还有其他地方引入了逗号。

让我们先确认修改是否完整，并提供最终版的修改方案，同时给出兼容旧数据的展示策略。

🔍 最终修复步骤
1. 确保 fmt_internal 已正确修改
在代码中找到 def prepare_report_data(): 上面的 def fmt_internal(v):，修改为：

python
def fmt_internal(v):
    if isinstance(v, list):
        # 列表内每个元素先替换英文逗号为中文逗号，再用顿号连接
        cleaned = [str(x).replace(",", "，") for x in v]
        return "、".join(cleaned)
    # 单值同样替换
    return str(v).replace(",", "，")
2. 确保 raw_data_str 构建正确
在 prepare_report_data 函数中，已有一行：

python
raw_data_str = ",".join(fmt_internal(ans.get(i, "")) for i in range(85))
这行没问题，但需确保没有其他地方又用未处理的值覆盖。

3. 详情页展示：兼容旧数据，显示更清晰
由于旧数据已错位，可增加一个简单的判断：如果 len(ans_list) < 85，说明旧数据被拆分过，我们可以尝试重新合并多选答案（针对旧数据做补救）。

但最简单的做法是：如果长度不为85，直接显示原始 raw_str，让用户自行对照，或者提示重新测评。
