...
st.markdown("""
    ...
    .stApp { 
        background: #F8F9FA; 
        overflow: hidden; /* 核心：禁止首页滑动 */
    }

    /* 首页专用遮盖容器： fixed 定位确保绝对居中 */
    .home-mask {
        position: fixed;
        top: 45%; 
        left: 50%;
        ...
        width: 92%;
        max-width: 500px;
        padding: 40px 25px 30px 25px; /* 底部留白减少，让按钮紧贴 */
    ...
    /* 调整按钮在 fixed 容器下的显示：让它也跟着定位 */
    div.stButton {
        text-align: center;
    }
    
    /* 首页按钮的特殊定位，确保它出现在卡片内部底部 */
    .home-btn-fixed {
        position: fixed;
        top: 68%; /* 对应卡片的底部位置 */
        left: 50%;
    ...
    .intro-text { font-size: 18px; color: #546E7A; line-height: 1.8; margin-bottom: 20px; border-left: 5px solid #FF7043; padding-left: 20px; }
...
if st.session_state.step == 'home':
    ...
    # 将按钮放入 fixed 定位容器，确保它叠在卡片之上且位置正确
    st.markdown("<div class='home-btn-fixed'>", unsafe_allow_html=True)
