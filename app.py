...
# D. 结果报告页逻辑
    ...
    if 'rid' not in st.session_state or not st.session_state.rid:
        ...
        st.write("⚠️ RID 丢失，已重新生成")
    
    st.write(f"🔍 RID = {st.session_state.rid}")
    st.write(f"🔍 ans 长度 = {len(st.session_state.ans) if st.session_state.ans else 0}")
