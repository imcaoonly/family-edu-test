...
st.markdown("""
    ...
    /* 1. 全系统动作按钮（开始测评、确认、提交）：深蓝满色 (#1A237E) */
    div.stButton > button {
    ...
    /* 3. 【回到上一题】按钮：修改为深蓝满色 (#1A237E)，视觉与动作按钮一致 (0brand) */
    /* 逻辑上，我们将其设为 primary 按钮 */
    .stApp button[kind="primary"] { 
        background-color: #1A237E !important; color: white !important; border: none !important;
        font-weight: 700 !important; height: 50px !important; border-radius: 12px !important;
    }

    /* 报告页：解析卡片层级颜色 (绿/橙/红) */
    .res-card { padding: 25px; border-radius: 18px; background: white; margin-bottom: 20px; line-height: 1.8; }
    .dim-lv0 { border-left: 8px solid #4CAF50; } /* 绿：稳固/优秀 */
    .dim-lv1 { border-left: 8px solid #FF9800; } /* 橙：内耗/预警 */
    .dim-lv2 { border-left: 8px solid #C62828; } /* 红：危险/力竭 */
    
    /* 转化卡片强化 */
    .wx-card { background: #FFFFFF; padding: 35px; border-radius: 24px; border: 2px solid #E8EAF6; box-shadow: 0 15px 45px rgba(26,35,126,0.18); text-align: left; margin-top: 40px; }
...
# --- 5. 1-6 维度原版全量长话术 (带三级颜色定义) ---
DIM_DATA = {
    "系统": {"range": range(0, 8), "texts": [
...
if st.session_state.step == 'home':
    ...
    """, unsafe_allow_html=True)
    if st.button("🚀 开始深度测评", key="start_main"):
...
# B. 答题页
    ...
    if cur < 78:
        ...
        q_info = BG_QUESTIONS[cur - 78]
        st.markdown(f"<div class='q-text'>{q_info['q']}</div>", unsafe_allow_html=True)
        if q_info['type'] == 'multi':
            selected = st.multiselect("可多选", q_info['opts'], key=f"m_{cur}")
            if st.button("确认进入下一题", key=f"next_{cur}", use_container_width=True):
            ...
            selected = st.radio("请选择", q_info['opts'], key=f"s_{cur}", index=None)
            if selected and st.button("确认进入下一题", key=f"next_{cur}", use_container_width=True):
                st.session_state.ans[cur] = selected
                if cur == 84: st.session_state.step = 'report'
                else: st.session_state.cur += 1
                st.rerun()

    # 底部统一深蓝满色“上一题”按钮，使用动态 key 修复报错
    if cur > 0:
        st.write("---")
        if st.button("⬅ 回到上一题", key=f"back_at_{cur}", kind="primary", use_container_width=True):
            st.session_state.cur -= 1
            st.rerun()
...
# C. 结果报告页
    ...
    scores = {n: sum(ans.get(i, 0) for i in d['range'])/len(d['range']) for n, d in DIM_DATA.items()}
    ...
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 3])), showlegend=False, height=400)
    ...
    st.markdown(f"""
        <div class='wx-card'>
            ...
            <div style='font-weight:bold; margin-top:25px; color:#1A237E; font-size:20px;'>添加微信您可以获得：</div>
            ...
            <div style='text-align:center; margin:35px 0;'>
                <div style='color:#C62828; font-size:15px; font-weight:700;'>专属报告编号 (截图保存)：</div>
                <div class='rid-box'>{st.session_state.rid}</div>
                <div style='color:#546E7A; font-size:14px; margin-top:10px;'>添加时请备注生成的数字</div>
            </div>
            <a href="https://work.weixin.qq.com/ca/cawcde91ed29d8de9f" target="_blank" 
               style="text-decoration:none; display:block; background:#1A237E; color:white; padding:22px; 
                      border-radius:18px; font-size:21px; font-weight:bold; text-align:center;">
                👉 点击添加曹校长，预约深度解析
            </a>
        </div>
